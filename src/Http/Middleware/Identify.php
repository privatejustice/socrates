...
class Identify implements Received, Sending {
  ...
  function identify($message, $driver, $user){
    ...
    try {
      $chatUser = socrates_api3('ChatUser', 'getsingle', [
        'service' => $service,
        'user_id' => $user->getId()
      ]);
  ...
  function createContact($user, $service){
    $contact = socrates_api3('Contact', 'create', [
    ...
    $result = socrates_api3('EntityTag', 'create', array(
  ...
  function createUser($service, $userId, $contactId){
    $result = socrates_api3('ChatUser', 'create', [
      'service' => $service,
      'user_id' => $userId,
