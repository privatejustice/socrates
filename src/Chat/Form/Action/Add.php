...
class Add extends \Socrates\Chat\Form\Good {
  ...
  function preProcessMassage(){
    ...
    Socrates\Core_Resources::singleton()->addScriptFile('chatbot', 'templates/CRM/Chat/Form/Action.js');
  ...
  function postProcessMassage($submitted){
    ...
    switch ($submitted['ChatAction:match']) {
      case 'Socrates\Chat\Check_Anything':
        $check = new Socrates\Chat\Check_Anything();
        break;

      case 'Socrates\Chat\Check_Contains':
        $check = new Socrates\Chat\Check_Contains(['contains' => $submitted['ChatAction:match_contains']]);
        break;

      case 'Socrates\Chat\Check_Equals':
        $check = new Socrates\Chat\Check_Equals(['equals'  => $submitted['ChatAction:match_equals']]);
        break;
    }
  ...
  public function validate() {
    ...
    if($submitted['ChatAction:match'] == 'Socrates\Chat\Check_Contains' && empty($submitted['ChatAction:match_contains'])){
      $this->_errors['ChatAction:match_contains'] = 'This field cannot be blank';
    }
    if($submitted['ChatAction:match'] == 'Socrates\Chat\Check_Equals' && empty($submitted['ChatAction:match_equals'])){
